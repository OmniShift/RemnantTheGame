...
io.on('connection', function (socket) {
                    ...
                    .on('row', function (row) {
                        ...
                        if (err) {
                            ...
                            if (hits === 0) {
                                ...
                                client.query(
                                    'INSERT INTO "GRIDs" (idname, status, playerid, playerready, playercommname, playerkingdompref) VALUES (\'' +
                                    gameRoomID +
                                    '\', 0, ARRAY[$$\'\'$$,$$\'\'$$,$$\'\'$$,$$\'\'$$], ARRAY[0,0,0,0], ARRAY[$$\'\'$$,$$\'\'$$,$$\'\'$$,$$\'\'$$], ARRAY[0,0,0,0]);',
                                    function (err, data) {
                                        if (err) {
                                            reject('failed to update lobbies');
                    ...
                    if (err) {
                        ...
                        if (status === 0) {
                            for (var i = 1; i < 4; i++) {
                                ...
                                if (row.playerid[i] === '\'\'') {
                                    client.query('UPDATE "GRIDs" SET playerid[' + i + '] = \'' + UID + '\' WHERE idname=\'' + roomID + '\';', function (
                                        err, data) {
                                        if (err) {
    ...
    socket.on('update lobby info', function (roomID, pNumber, pID, pReady, pCommName, pKingdomPref) {
        console.log('Room ID: ' + roomID + ', sent player number: ' + pNumber + ', sent player ID: ' + pID + ', sent player ready: ' + pReady +
        ...
        new Promise(function (resolve, reject) {
            ...
            pg.connect(process.env.DATABASE_URL, function (err, pclient) {
                ...
                pclient.query('UPDATE "GRIDs" SET playerid[' + pNumber + '] = \'' + pID + '\', playerready[' + pNumber + '] = ' + pReady +
                    ', playercommname[' + pNumber + '] = \'' + pCommName + '\', playerkingdompref[' + pNumber + '] = ' + pKingdomPref +
                    ' WHERE idname=\'' + roomID + '\';',
                    function (err, data) {
                        if (err) {
